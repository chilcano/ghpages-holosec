...
---
[En la anterior entrada](/2014/05/14/mtom-en-wso2-esb-para-optimizar-la-transferencia-de-datos-binarios-sobre-soap-parte1/ "MTOM en WSO2 ESB para optimizar la transferencia de datos binarios sobre SOAP (Parte 1/2)") explorábamos los beneficios del uso de MTOM en WSO2 ESB, ahora explicaremos cómo integrar esta funcionalidad en nuestras aplicaciones web, es decir, cómo invocar el mismo servicio desde una página web tradicional.

![](/assets/blog20140526_wso2_mtom_2/wso2-esb.jpg)
...
![](/assets/blog20140526_wso2_mtom_2/wso2esb-mtom-part2-http2soap-01.png)
_HTTP Multipart/form-data a MTOM SOAP en WSO2 ESB_
...
En la anterior entrada implementamos un proxy para el envio de ficheros usando MTOM y SOAP. Entonces, vamos a reutilizar el mismo proxy y añadiremos unos pequeños cambios.Este proxy es un proxy actualizado a partir del proxy publicado en la entrada anterior "[MTOM en WSO2 ESB para optimizar la transferencia de datos binarios sobre SOAP (Parte 1/2)](/2014/05/14/mtom-en-wso2-esb-para-optimizar-la-transferencia-de-datos-binarios-sobre-soap-parte1/ "MTOM en WSO2 ESB para optimizar la transferencia de datos binarios sobre SOAP (Parte 1/2)")". Sabemos que en el lado servidor tenemos que desplegar la implementación de este servicio para que todo funcione. Más adelante explicaré que es necesario tocar algunas líneas de código.
...
<proxy
	...
	<target>
		<inSequence>
			...
			<send>
				<endpoint>
					<address uri="http://localhost:8281/services/MTOMSwASampleService_proxy"  format="soap11"/>
				</endpoint>
			</send>
			<log level="custom">
...
5. Para verificar que hemos implementado una nueva operación, podemos consultar el actualizado `WSDL` en la siguiente dirección: [http://localhost:9000/services/MTOMSwASampleService?wsdl](http://localhost:9000/services/MTOMSwASampleService?wsdl)
...
![](/assets/blog20140526_wso2_mtom_2/wso2esb-mtom-part2-http2soap-02form.png)
