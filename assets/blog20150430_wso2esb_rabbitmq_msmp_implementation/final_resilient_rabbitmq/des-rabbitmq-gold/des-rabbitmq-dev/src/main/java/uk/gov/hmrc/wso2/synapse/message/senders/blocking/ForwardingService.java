...
//import uk.gov.hmrc.wso2.synapse.senders.blocking.BlockingMsgSender;
...
public class ForwardingService implements InterruptableJob, Service {
    ...
    // N/A, instead of this, this is RabbitMQCustomTaskReceiver
    //private MessageProcessor messageProcessor;
    private boolean msgTaskProcessorIsDeactived = false;
    
    /** This is the client which sends messages to the BACKEND endpoint */
    private BlockingMsgSender blockingBackendSender;
    ...
    /** Endpoint to be used to send the RabbitMQ's MSG */
    private String targetEndpoint = null;

    /** This is specially used for REST scenarios where HTTP_SC can take semantics in a RESTful architecture. */
    ...
    /** Number of retries before shutting-down the processor. -1 default value indicates that retry should happen forever */
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        do {
            ...
            try {
            	...
            	if (!this.msgTaskProcessorIsDeactived) {
                    ...
                    if (messageContext != null) {
                        ...
                        if(serverName != null && messageContext instanceof Axis2MessageContext) {
                            AxisConfiguration configuration = ((Axis2MessageContext)messageContext).getAxis2MessageContext().getConfigurationContext().getAxisConfiguration();
    ...
    public boolean init(JobExecutionContext jobExecutionContext) {
        ...
        blockingBackendSender = (BlockingMsgSender) jdm.get(ScheduledMessageForwardingProcessor.BLOCKING_SENDER);
        ...
        if (parameters.get(ForwardingProcessorConstants.REPLY_SEQUENCE) != null) {
            ...
            log.debug("~ init() ~ replySeq " + replySeq);

        } else {
        	log.debug("~ init() ~ REPLY_SEQUENCE is null");
        }
    ...
    public boolean dispatch(MessageContext messageContext) {
    	...
    	log.debug("~ dispatch(messageContext) ~ start");
        ...
        if (targetEndpoint != null) {
            ...
            try {
                ...
                while (!isSuccessful && !isTerminated) {
                    try {
                        ...
                        outCtx = blockingBackendSender.send(ep, messageContext); // ***************************
                    ...
                    log.debug("~ dispatch(messageContext) ~ isSuccessful 1 : " + isSuccessful);
                    
                    if (isSuccessful) {
                        if (outCtx != null) {
                        	log.debug("~ dispatch(messageContext) ~ outCtx in not null.");
                        	log.debug("~ dispatch(messageContext) ~ outCtx.getProperty(ForwardingProcessorConstants.BLOCKING_SENDER_ERROR) : " + outCtx.getProperty(ForwardingProcessorConstants.BLOCKING_SENDER_ERROR));
                        	
                            if ("true".equals(outCtx.getProperty(ForwardingProcessorConstants.BLOCKING_SENDER_ERROR))) {

                            	String strErrorInOutBE = (String) outCtx.getProperty(SynapseConstants.ERROR_MESSAGE);
                            	log.debug("~ dispatch(messageContext) ~ the ReplySeq receive this error. strErrorInOutBE = " + strErrorInOutBE);
                            	
                                // this means send has failed due to some reason so we have to retry it
                                isSuccessful = isNonRetryErrorCode((String) outCtx.getProperty(SynapseConstants.ERROR_MESSAGE));
                                ...
                                if (log.isDebugEnabled()) {
                                    log.debug("Successfully sent the message to endpoint [" + ep.getName() +"]" + " with message processor [" + messageProcessor.getName() + "]");
                                }
                            }
                            
                        } else {
                        	log.debug("~ dispatch(messageContext) ~ This Means we have invoked an out only operation, remove the message and reset the count.");
                    ...
                    log.debug("~ dispatch(messageContext) ~ isSuccessful 2 : " + isSuccessful);
            ...
            log.warn("Property " + ForwardingProcessorConstants.TARGET_ENDPOINT + " not found in the message context , Hence removing the message ");
    ...
    public void sendThroughFaultSeq(MessageContext msgCtx) {
    	log.debug("~~ sendThroughFaultSeq(outCtx) ~ begin");
    	log.debug("~~ sendThroughFaultSeq(outCtx) ~ faultSeq : " + faultSeq);
        if (faultSeq == null) {
    ...
    public void sendThroughReplySeq(MessageContext outCtx) {
    	log.debug("~~ sendThroughReplySeq(outCtx) ~ begin");
    	log.debug("~~ sendThroughReplySeq(outCtx) ~ replySeq : " + replySeq);
        if (replySeq == null) {
            this.messageProcessor.deactivate();
            log.error("Can't Send the Out Message , Sequence name " + replySeq + " does not Exist. Deactivated the" + " message processor");
            return;
        }
        ...
        if (mediator == null) {
            this.messageProcessor.deactivate();
            log.error("Can't Send the Out Message , Sequence object " + replySeq + " does not Exist. Deactivated the" + " message processor");
            return;
        }
    ...
    private void resetService() {
    	log.debug("resetService() - start");
        isSuccessful = false;
        attemptCount = 0;
    }

    private boolean isNonRetryErrorCode(String errorMsg) {
    	log.debug("~ isNonRetryErrorCode() ~ errorMsg : " + errorMsg);
