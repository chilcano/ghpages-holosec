"""html_wp_to_md: Simple Python script to convert downloaded Wordpress.com HTML into equivalent Markdown-structured text."""
...
def gen_markdown(html_content):
	...
	# for preserving the line break
	html_part = re.sub(r"\n", r"<br/>", html_part, re.UNICODE)
	# convert html to md
	markdown_part = h.handle(html_part)

	# for replacing [pre lang=pqr]xyz[/pre] for ```xyz```
	markdown_part = re.sub(r"\[pre lang=([a-zA-Z0-9]*)\](.*?)\[\/pre\]", r"```\1\2```", markdown_part, flags=re.UNICODE|re.MULTILINE|re.DOTALL)

	# for removing blank lines
	#markdown_part = re.sub(r"\n\s*\n\t?\n?", r"\n", markdown_part, re.UNICODE)
	#print("\t\tNo. of matches:", len( re.findall(r"\n\s*\n", markdown_part) ))
...
def main():
	...
	if len(sys.argv) == 1:
		print("Usage:")
		print("\t$ python html_wp_to_md.py <input-html-wordpress-dir>, <optional-markdown-output-dir>\n")
		return
	elif len(sys.argv) == 2:
